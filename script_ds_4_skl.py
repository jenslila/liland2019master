...
nanodir = "data/ncbi/nanoporecounts/"
# }}}
...
if __name__ == '__main__':
    ...
    ### Trim nanolocation based on what exists atm
#   outpath <- paste0(outdir, "a_K=", K, "_",
#                     accession, ".csv")
    prod = pd.DataFrame(it.product(["a", "c"], KLengthsART,
                                   nanolocation["accession"]))
    paths = \
    prod.apply(lambda x: \
        "data/ncbi/nanoporecounts/{}_K={}_{}.csv.xz".format(x[0],
            x[1], x[2]), axis=1).values
    exists = np.vectorize(os.path.isfile)(paths)
    missingaccs = prod.loc[np.invert(exists)][2].values
    idx = np.vectorize(lambda x: np.isin(x, missingaccs))(nanolocation["accession"])
    nanolocation = nanolocation.loc[np.invert(idx)]
    idx = \
    np.vectorize(lambda x: \
      len(np.unique(nanolocation.loc[nanolocation["system"]==x]["folder"])
          ))(np.unique(nanolocation["system"]))==2
    systemstoomit = np.unique(nanolocation["system"])[idx]
    idx = np.vectorize(lambda x: np.isin(x,
        systemstoomit))(nanolocation["system"])
    nanolocation = nanolocation.loc[idx]
    # }}}

    # {{{ IsPlasmid column on location and artlocation
    location["IsPlasmid"] = IsPlasmid(location["folder"])
    nanolocation["IsPlasmid"] = IsPlasmid(nanolocation["folder"])
    # }}}

    # {{{ artlocation derived variables
    genomes = np.unique(nanolocation["system"])
    accessions = nanolocation["accession"].values
    Yart = nanolocation["folder"].values
    # }}}
    ...
    for KLengthART, group in it.product(KLengthsART, groups):
        ...
        for genome in genomes:
            savepathresults = mesgresultsnano.format(group, KLengthART,
                                                     genome)
            savepathwatches = mesgwatchesnano.format(group, KLengthART,
                                                     genome)
            if not os.path.isfile(savepathresults):
                ...
                IdxTest = nanolocation["system"] == genome
                Ytest = nanolocation.loc[IdxTest, "folder"].values
                Ynumtest = nanolocation.loc[IdxTest, "IsPlasmid"].values
                AccessionsTest = nanolocation.loc[IdxTest,
                                                  "accession"].values
                # }}}
